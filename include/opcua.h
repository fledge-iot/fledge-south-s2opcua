...
public:
	...
	bool write(const std::string &nodeId, const std::string &value);
...
private:
	...
	static void asyncS2ResponseCallBack(SOPC_EncodeableType *encType, const void *response, uintptr_t appContext);

	bool read(const char *nodeIdStr, SOPC_BuiltinId *outBuiltinTypeId, SOPC_VariantArrayType *outArrayType);
	SOPC_DataValue *toDataValue(SOPC_BuiltinId builtinId, const char *val);
	...
	OpcUa_GetEndpointsResponse *GetEndPoints(const char *endPointUrl);
	...
	char **m_nodeIds;
	size_t m_numNodeIds;
	std::vector<std::string> m_subscriptions; // The user subscriptions
	std::map<std::string, Node *> m_nodes;	 // The nodes being monitored
	std::string m_url;
	std::string m_asset;
	...
	bool m_init;
	std::map<std::string, struct timeval> m_lastIngest;
	enum
	{
		ASSET_NAME_SINGLE,
		ASSET_NAME_SINGLE_OBJ,
		ASSET_NAME_OBJECT,
		ASSET_NAME
	} m_assetNaming;
	bool m_dpNameIsNodeId;
	std::set<Node *> m_nodeObjects;
	std::map<std::string, std::string> m_parents; // Map variable node id to parent node id
	std::map<std::string, Node *> m_parentNodes;
	std::map<std::string, std::string> m_fullPaths; // Map variable node id to full OPC UA path
	...
	std::string getFilterScopeStr()
	{
		...
		{
		case NodeFilterScope::SCOPE_OBJECT:
			return "Object";
		case NodeFilterScope::SCOPE_VARIABLE:
			return "Variable";
		case NodeFilterScope::SCOPE_OBJECT_VARIABLE:
			return "Object and Variable";
		default:
	...
	std::string getFilterActionStr()
	{
		...
		{
		case NodeFilterAction::INCLUDE:
			return "Include";
		case NodeFilterAction::EXCLUDE:
			return "Exclude";
		default:
